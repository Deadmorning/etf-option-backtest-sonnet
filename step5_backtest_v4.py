# -*- coding: utf-8 -*-
"""
Step 5 v4: ETF期权三阶段策略 — 四项优化版
优化清单（相对v3.2）：
  Opt2: 入场延迟到09:45, 需确认开盘方向仍有效（防开盘陷阱）
  Opt3: 反手阈值 0.5% → 0.8%（减少噪声反手）
  Opt4: 期权止损：权利金跌幅>35%时主动平仓
  Opt5: 对冲需连续2个15分钟确认浮盈>5%才执行
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(OUTPUT_DIR, "backtest_v4.log"),
...
REV_THRESH    = 0.008    # [OPT3] 偏离>0.8% 反手（原0.5%）
PROFIT_THRESH = 0.05     # 盈利>5% 对冲
ROLL_DAYS     = 5        # 距到期≤5交易日换月
ENTRY_TIME    = "09:45"  # [OPT2] 延迟入场确认时间（原09:30）
STOP_LOSS_PCT = 0.35     # [OPT4] 权利金跌幅>35%止损
HEDGE_CONFIRM_N = 2      # [OPT5] 对冲需连续N次确认（原1次）
...
class IntradayBacktesterV4:
    ...
    def run_day(self, idx: int) -> dict:
        ...
        hedge_confirm = 0          # [OPT5] 对冲连续确认计数
        capital_at_open = self.capital

        # ── [OPT2] 09:30 仅计算开盘信号，不立即入场 ──────────────
    ...
    def run(self):
        logger.info(f"=== 创业板ETF期权 日内三阶段 v4 ===  资金={INIT_CAPITAL:,.0f}")
        logger.info(f"优化: 入场延迟={ENTRY_TIME}, 反手阈值={REV_THRESH*100:.1f}%, "
                    f"止损={STOP_LOSS_PCT*100:.0f}%, 对冲确认={HEDGE_CONFIRM_N}次")
        for i in range(1, len(self.etf)):
            r = self.run_day(i)
            logger.info(f"{r['date']}  [{r['expiry_label']}]  信号={r['signal']:<4} "
...
print()
bt = IntradayBacktesterV4(etf)
daily_df, trades_df = bt.run()

# ── 绩效报告 ──────────────────────────────────────────────────
print("\n" + "═"*64)
print("      创业板ETF期权 三阶段策略 v4（四项优化）回测报告")
print("═"*64)
...
n_sl    = (trades_df["action"]=="止损平仓").sum()
...
print(f"\n【风险收益】")
print(f"  夏普比率: {sharpe:>8.4f}    最大回撤: {max_dd*100:>8.2f}%")
print(f"\n【交易统计】")
print(f"  总笔数:{len(trades_df):>5}  开仓:{n_open}  反手:{n_rev}  对冲:{n_hedge}  止损:{n_sl}")
print(f"  胜率: {win_rate*100:.1f}%   盈亏比: {plr:.3f}")
print(f"  止损触发: {daily_df['stop_loss'].sum()}天")
...
perf = {
    ...
    "n_hedge": int(n_hedge), "n_stoploss": int(n_sl),
}
daily_df.to_csv(f"{OUTPUT_DIR}/backtest_v4_daily.csv",   index=False, encoding="utf-8-sig")
trades_df.to_csv(f"{OUTPUT_DIR}/backtest_v4_trades.csv", index=False, encoding="utf-8-sig")
monthly.to_csv(f"{OUTPUT_DIR}/backtest_v4_monthly.csv",  index=False, encoding="utf-8-sig")
with open(f"{OUTPUT_DIR}/backtest_v4_perf.json","w") as f:
    json.dump(perf, f, indent=2)
etf.to_csv(f"{OUTPUT_DIR}/etf_159915_daily_2026Q1.csv", index=False, encoding="utf-8-sig")

print(f"\n✓ 已保存: backtest_v4_{{daily,trades,monthly,perf}}")
